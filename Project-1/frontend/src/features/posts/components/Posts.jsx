...
const Posts = ({user , post}) => {
  const { handleLike, handleUnlike ,Handlefollow , HandleUnfollow } = useAuth()
  return (
        <div className="post">
            <div className="user">
                ...
                <p>{user?.name}</p>
                <button onClick={() => { post.isFollowing ? HandleUnfollow(user?.name) : Handlefollow(user?.name) }} className={post.isFollowing ? "btn following" : "btn"}>{post.isFollowing ? "Following" : "Follow"}</button>
            </div>
            <img src={post.imgUrl} alt="" />
            <div className="icons">
                <div className="left">
                    <button onClick={() => { post.isliked ? handleUnlike(post._id) : handleLike(post._id) }}><svg
